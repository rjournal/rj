...
- Make sure to clone the `rjournal.github.io` repo parallel to the
  `articles` repo.
...
- When an article is ready, copy to the `rjournal.github.io` repo using
  ...
  Note that the volume is the current year minus 2008. This will also
  assign a “slug” (the id of the published article), which is added to
  the DESCRIPTION file. This slug will be used for the folder name in
...
- Check the issue carefully yourself at
  `https://rjournal.github.io/issues/<issue id>`.
