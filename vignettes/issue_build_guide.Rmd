...
* For each article, this will also create a zip file of supplementary material from the list of files in the DESCRIPTION.
...
# Assembling the issue
  ...
  * When an article is ready, copy to the `rjournal.github.io` repo using 
    ...
    ```
    This will also assign a "slug" (the id of the published article), which is added to the DESCRIPTION file. This slug will be used for the folder name in the `rjournal.github.io` "_articles" folder.
  * For any news item: 
    ```r
